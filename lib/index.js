	...
	*/
	...
	async fire(pending) {
		...
		if (outcome === "delivered") {
			pending.pushes += 1;
			entry.retries = 0;
			if (this.repeatMs() > 0) entry.repeat = this.after(this.repeatMs(), () => {
				this.fire(pending);
			});
			return;
		}
		if (outcome === "deferred") {
			if (this.retryMs() > 0) entry.repeat = this.after(this.retryMs(), () => {
				this.fire(pending);
			});
			return;
		}
		if (outcome === "retryable-failure") {
			if (entry.retries < 2 && this.retryMs() > 0) {
				entry.retries += 1;
				entry.repeat = this.after(this.retryMs(), () => {
					this.fire(pending);
				});
			} else this.entries.delete(pending.id);
			return;
		}
...
function buildPushUrl(credential) {
	...
	if (/^https:\/\//i.test(value)) {
		...
		if (parsed.hostname === "sctapi.ftqq.com" && classic !== null && !/^SCTP/i.test(classic[1])) return `https://sctapi.ftqq.com/${classic[1]}.send`;
		const sc3 = /^\/send\/(sctp(\d+)t[A-Za-z0-9_-]+)\.send\/?$/.exec(parsed.pathname);
		if (sc3 !== null && parsed.hostname === `${sc3[2]}.push.ft07.com`) return `https://${sc3[2]}.push.ft07.com/send/${sc3[1]}.send`;
		return null;
	}
...
async function sendPush(url, proxy, title, desp, fetchImpl = fetch) {
	...
	let dispatcher;
	try {
		dispatcher = proxy !== "" ? new ProxyAgent(proxy) : void 0;
		...
		if (code !== 0) return {
			ok: false,
			message: "server code indicates failure",
			outcome: "terminal-failure"
		};
		return {
			ok: true,
		...
		return {
			ok: false,
			message: error instanceof Error && (error.name === "AbortError" || error.name === "TimeoutError") ? "timeout" : "network-failed",
			outcome: "retryable-failure"
		};
	} finally {
...
function apply(ctx, config) {
	...
	if (sessions !== void 0 && settings().enabled && settings().thresholdMinutes > 0) for (const session of sessions.list()) for (const seed of recoverPending(session.events, session.id)) {
		tracker.start(seed);
