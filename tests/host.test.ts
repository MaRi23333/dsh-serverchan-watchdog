...
test('guardLoopback normalizes IPv4-mapped IPv6 and rejects LAN peers', () => {
  assert.equal(guardLoopback(fakeReq('127.0.0.1', {}), fakeRes), true)
  assert.equal(guardLoopback(fakeReq('::1', {}), fakeRes), true)
  assert.equal(guardLoopback(fakeReq('::ffff:127.0.0.1', {}), fakeRes), true)
  assert.equal(guardLoopback(fakeReq('::FFFF:127.0.0.1', {}), fakeRes), true)
...
test('guardWrite requires loopback + JSON + same-origin Host match', () => {
  ...
  assert.equal(guardWrite(fakeReq('127.0.0.1', { 'content-type': 'text/plain' }), fakeRes), false)
})
...
test('sendPush treats non-zero server code and non-200 status as failure', async () => {
  ...
  assert.equal(codeResult.message, 'server code 40001')
